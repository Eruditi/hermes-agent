...
批处理允许您在数百或数千个提示上并行运行 Hermes 代理，生成结构化轨迹数据。这主要用于 **训练数据生成** — 生成带有工具使用统计信息的 ShareGPT 格式轨迹，可用于微调或评估。

## 概述

批处理运行器（`batch_runner.py`）处理提示的 JSONL 数据集，通过完整的代理会话运行每个提示，并具有工具访问权限。每个提示都有自己的隔离环境。输出是结构化轨迹数据，包含完整的对话历史、工具调用统计信息和推理覆盖指标。
...
| 参数 | 描述 |
|-----------|-------------|
| `--providers_allowed` | 允许的逗号分隔提供程序（例如 `"anthropic,openai"`） |
| `--providers_ignored` | 要忽略的逗号分隔提供程序（例如 `"together,deepinfra"`） |
| `--providers_order` | 逗号分隔的首选提供程序顺序 |
...
每个提示从 **分布** 中获得随机采样的工具集。这确保训练数据涵盖多样化的工具组合。使用 `--list_distributions` 查看所有可用的分布。

在当前实现中，分布为 **每个单独的工具集** 分配一个概率。采样器独立翻转每个工具集，然后保证至少启用一个工具集。这与手动制作的预构建组合表不同。
...
`conversations` 字段使用类似 ShareGPT 的格式，带有 `from` 和 `value` 字段。工具统计信息被归一化以包括所有可能的工具，零作为默认值，确保 HuggingFace 数据集兼容性的条目间一致的模式。
...
- **基于内容的恢复：** 在 `--resume` 上，运行器扫描现有的批处理文件，并通过其实际文本内容（而不仅仅是索引）匹配已完成的提示，即使数据集顺序发生变化也能恢复
- **失败的提示：** 只有成功完成的提示才会被标记为已完成 — 失败的提示将在恢复时重试
- **批处理合并：** 完成后，所有批处理文件（包括来自先前运行的文件）都合并到单个 `trajectories.jsonl` 中
...
- **无推理过滤器：** 零个助手回合包含推理（无 `<REASONING_SCRATCHPAD>` 或本机思考令牌）的样本将被丢弃
- **损坏条目过滤器：** 在最终合并期间过滤掉带有幻觉工具名称（不在有效工具列表中）的条目
- **推理统计信息：** 跟踪整个运行中带有/不带推理的回合百分比
...
统计信息也保存到 `statistics.json` 以便进行编程分析。
