...
| `--dev` | 与 `--tui` 一起使用：通过 `tsx` 直接运行 TypeScript 源代码，而不是预构建的捆绑包（适用于 TUI 贡献者）。 |

## 顶级命令

| 命令 | 用途 |
|---------|---------|
| `hermes chat` | 与代理的交互式或一次性聊天。 |
...
| `hermes acp` | 运行 Hermes 作为 ACP 服务器，用于编辑器集成。 |
...
| `--provider <provider>` | 强制提供商：`auto`、`openrouter`、`nous`、`openai-codex`、`copilot-acp`、`copilot`、`anthropic`、`gemini`、`google-gemini-cli`、`huggingface`、`zai`、`kimi-coding`、`minimax`、`minimax-cn`、`kilocode`、`xiaomi`、`arcee`、`alibaba`、`deepseek`、`nvidia`、`ollama-cloud`、`xai`（别名 `grok`）。 |
| `-s`, `--skills <name>` | 为会话预加载一个或多个技能（可重复或逗号分隔）。 |
| `-v`, `--verbose` | 详细输出。 |
| `-Q`, `--quiet` | 编程模式：抑制横幅/ spinner/ 工具预览。 |
...
交互式提供商 + 模型选择器。**这是用于添加新提供商、设置 API 密钥和运行 OAuth 流程的命令**。从您的终端运行它 — 不是从活动的 Hermes 聊天会话中。
...
**`hermes model`**（从您的终端运行，在任何 Hermes 会话之外）是**完整的提供商设置向导**。它可以添加新提供商、运行 OAuth 流程、提示输入 API 密钥和配置端点。

**`/model`**（在活动的 Hermes 聊天会话中键入）只能**在您已设置的提供商和模型之间切换**。它不能添加新提供商、运行 OAuth 或提示输入 API 密钥。
...
默认情况下，`/model` 更改**仅适用于当前会话**。添加 `--global` 将更改持久化到 `config.yaml`：
...
如果您只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加另一个提供商（Anthropic、DeepSeek、Copilot 等），退出会话并从终端运行 `hermes model`。
:::

提供商和基础 URL 更改会自动持久化到 `config.yaml`。当从自定义端点切换时，过时的基础 URL 会被清除，以防止它泄漏到其他提供商中。
...
:::tip WSL 用户
使用 `hermes gateway run` 而不是 `hermes gateway start` — WSL 的 systemd 支持不可靠。使用 tmux 包装以保持持久：`tmux new -s hermes 'hermes gateway run'`。有关详细信息，请参阅 [WSL FAQ](/docs/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
|------------|-------------|
| `list` | 显示计划作业。 |
| `create` / `add` | 从提示创建计划作业，可选通过重复的 `--skill` 附加一个或多个技能。 |
| `edit` | 更新作业的计划、提示、名称、交付、重复计数或附加的技能。支持 `--clear-skills`、`--add-skill` 和 `--remove-skill`。 |
| `pause` | 暂停作业而不删除它。 |
| `resume` | 恢复暂停的作业并计算其下一次未来运行。 |
| `run` | 在下次调度程序触发时触发作业。 |
| `remove` | 删除计划作业。 |
| `status` | 检查 cron 调度程序是否运行。 |
| `tick` | 运行到期作业一次并退出。 |
...
输出您整个 Hermes 设置的紧凑、纯文本摘要。设计用于在寻求支持时粘贴到 Discord、GitHub issues 或 Telegram 中 — 无 ANSI 颜色，无特殊格式，只有数据。
...
:::tip
`hermes dump` 专门设计用于共享。对于交互式诊断，使用 `hermes doctor`。对于视觉概览，使用 `hermes status`。
:::
...
上传调试报告（系统信息 + 最近的日志）到粘贴服务并获取可共享的 URL。对于快速支持请求很有用 — 包含帮助者诊断您问题所需的一切。
...
报告包括系统信息（OS、Python 版本、Hermes 版本）、最近的代理和网关日志（每个文件 512 KB 限制）以及编辑的 API 密钥状态。密钥始终被编辑 — 不会上传任何秘密。
...
创建您的 Hermes 配置、技能、会话和数据的 zip 存档。备份不包括 hermes-agent 代码库本身。
...
当 `--since` 处于活动状态时，包含没有可解析时间戳的行（它们可能是多行日志条目的延续行）。当 `--level` 处于活动状态时，包含没有可检测级别的行。
...
启动 Hermes 作为 ACP（代理客户端协议）stdio 服务器，用于编辑器集成。
...
| 子命令 | 描述 |
|------------|-------------|
| `serve [-v\|--verbose]` | 运行 Hermes 作为 MCP 服务器 — 向其他代理公开对话。 |
...
统一插件管理 — 通用插件、内存提供商和上下文引擎在一个地方。运行不带子命令的 `hermes plugins` 会打开一个复合交互式屏幕，包含两个部分：

- **通用插件** — 多选复选框，用于启用/禁用已安装的插件
- **提供商插件** — 内存提供商和上下文引擎的单选配置。在类别上按 ENTER 打开单选选择器。
...
将您的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测遗留目录名称（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
| `--workspace-target <path>` | 工作空间指令的目标目录（AGENTS.md）。 |
| `--skill-conflict <mode>` | 处理技能名称冲突：`skip`（默认）、`overwrite` 或 `rename`。 |
| `--yes` | 跳过确认提示。 |

### 迁移内容

迁移涵盖 30+ 个类别，包括角色、内存、技能、模型提供商、消息平台、代理行为、会话策略、MCP 服务器、TTS 等。项目要么**直接导入**到 Hermes 等价物中，要么**存档**以供手动审查。

**直接导入：** SOUL.md、MEMORY.md、USER.md、AGENTS.md、技能（4 个源目录）、默认模型、自定义提供商、MCP 服务器、消息平台令牌和允许列表（Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost）、代理默认值（推理努力、压缩、人类延迟、时区、沙箱）、会话重置策略、批准规则、TTS 配置、浏览器设置、工具设置、执行超时、命令允许列表、网关配置和来自 3 个源的 API 密钥。

**存档以供手动审查：** Cron 作业、插件、钩子/webhook、内存后端（QMD）、技能注册表配置、UI/身份、日志记录、多代理设置、频道绑定、IDENTITY.md、TOOLS.md、HEARTBEAT.md、BOOTSTRAP.md。
...
将 shell 补全脚本打印到 stdout。在 shell 配置文件中获取输出来获得 Hermes 命令、子命令和配置文件名称的制表符补全。
