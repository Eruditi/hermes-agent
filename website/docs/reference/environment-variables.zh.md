...
| `COPILOT_GITHUB_TOKEN` | Copilot API 的 GitHub 令牌 — 第一优先级（OAuth `gho_*` 或细粒度 PAT `github_pat_*`；经典 PAT `ghp_*` **不受支持**） |
...
| `HF_TOKEN` | 推理提供者的 Hugging Face 令牌 ([huggingface.co/settings/tokens](https://huggingface.co/settings/tokens)) |
...
| `OLLAMA_API_KEY` | Ollama Cloud API 密钥 — 无本地 GPU 的托管 Ollama 目录 ([ollama.com/settings/keys](https://ollama.com/settings/keys)) |
...
| `OPENCODE_GO_API_KEY` | OpenCode Go API 密钥 — 10 美元/月的开放模型订阅 ([opencode.ai](https://opencode.ai/auth)) |
| `OPENCODE_GO_BASE_URL` | 覆盖 OpenCode Go 基础 URL |
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果您手动导出一个，则显式 Claude Code 令牌覆盖 |
| `HERMES_MODEL` | 在进程级别覆盖模型名称（被 cron 调度器使用；正常使用时首选 `config.yaml`） |
...
| `HERMES_HOME` | 覆盖 Hermes 配置目录（默认：`~/.hermes`）。还作用域网关 PID 文件和 systemd 服务名称，因此多个安装可以并发运行 |

## 提供者认证（OAuth）

对于原生的 Anthropic 认证，当 Claude Code 自己的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。诸如 `ANTHROPIC_TOKEN` 之类的环境变量仍然可用作手动覆盖，但它们不再是 Claude Pro/Max 登录的首选路径。
...
| `TAVILY_API_KEY` | 用于 AI 原生网页搜索、提取和抓取的 Tavily API 密钥 ([app.tavily.com](https://app.tavily.com/home)) |
...
| `SUPERMEMORY_API_KEY` | 带有配置文件召回和会话摄取的语义长期内存 ([supermemory.ai](https://supermemory.ai)) |
...
| `TERMINAL_DOCKER_FORWARD_ENV` | 要显式转发到 Docker 终端会话的环境变量名称的 JSON 数组。注意：技能声明的 `required_environment_variables` 会自动转发 — 您只需要这个用于未被任何技能声明的变量。 |
| `TERMINAL_DOCKER_VOLUMES` | 额外的 Docker 卷挂载（逗号分隔的 `host:container` 对） |
| `TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE` | 高级选择：将启动 cwd 挂载到 Docker `/workspace`（`true`/`false`，默认：`false`） |
...
| `SUDO_PASSWORD` | 无需交互式提示启用 sudo |

对于云沙箱后端，持久化是面向文件系统的。`TERMINAL_LIFETIME_SECONDS` 控制 Hermes 何时清理空闲终端会话，稍后的恢复可能会重新创建沙箱而不是保持相同的实时进程运行。
...
| `TELEGRAM_HOME_CHANNEL` | cron 传递的默认 Telegram 聊天/频道 |
...
| `TELEGRAM_IGNORED_THREADS` | 机器人从不响应的逗号分隔 Telegram 论坛主题/线程 ID |
...
| `DISCORD_ALLOWED_ROLES` | 允许使用机器人的逗号分隔 Discord 角色 ID（与 `DISCORD_ALLOWED_USERS` 或）。自动启用 Members 意图。当审核团队轮换时很有用 — 角色授权自动传播。 |
| `DISCORD_HOME_CHANNEL` | cron 传递的默认 Discord 频道 |
...
| `DISCORD_NO_THREAD_CHANNELS` | 机器人响应而不自动线程化的逗号分隔频道 ID |
...
| `SLACK_HOME_CHANNEL` | cron 传递的默认 Slack 频道 |
...
| `WHATSAPP_ALLOWED_USERS` | 逗号分隔的电话号码（带国家代码，无 `+`），或 `*` 允许所有发件人 |
| `WHATSAPP_ALLOW_ALL_USERS` | 允许所有 WhatsApp 发件人而无需允许列表（`true`/`false`） |
...
| `TWILIO_ACCOUNT_SID` | Twilio 帐户 SID（与电话技能共享） |
...
| `FEISHU_HOME_CHANNEL` | cron 传递和通知的飞书聊天 ID |
...
| `WECOM_HOME_CHANNEL` | cron 传递和通知的企业微信聊天 ID |
...
| `WECOM_CALLBACK_ALLOW_ALL_USERS` | 设置 `true` 以允许所有用户而无需允许列表 |
| `WEIXIN_ACCOUNT_ID` | 通过 iLink Bot API 二维码登录获得的微信帐户 ID |
| `WEIXIN_TOKEN` | 通过 iLink Bot API 二维码登录获得的微信身份验证令牌 |
...
| `WEIXIN_GROUP_POLICY` | 组消息策略：`open`、`allowlist`、`disabled`（默认：`disabled`） |
| `WEIXIN_ALLOWED_USERS` | 允许 DM 机器人的逗号分隔微信用户 ID |
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔微信群组 ID |
| `WEIXIN_HOME_CHANNEL` | cron 传递和通知的微信聊天 ID |
...
| `BLUEBUBBLES_HOME_CHANNEL` | cron/通知传递的电话/电子邮件 |
...
| `QQ_GROUP_ALLOWED_USERS` | 用于组 @-消息访问的逗号分隔 QQ 组 ID |
| `QQ_ALLOW_ALL_USERS` | 允许所有用户（`true`/`false`，覆盖 `QQ_ALLOWED_USERS`） |
| `QQBOT_HOME_CHANNEL` | cron 传递和通知的 QQ 用户/组 openID |
...
| `MATTERMOST_HOME_CHANNEL` | 主动消息传递（cron、通知）的频道 ID |
...
| `MATTERMOST_REPLY_MODE` | 回复样式：`thread`（线程化回复）或 `off`（平面消息，默认） |
...
| `MATRIX_HOME_ROOM` | 主动消息传递的房间 ID（例如 `!abc123:matrix.org`） |
...
| `MATRIX_DM_MENTION_THREADS` | 当在 DM 中 `@mentioned` 机器人时创建线程（默认：`false`） |
| `MATRIX_RECOVERY_KEY` | 设备密钥轮换后用于交叉签名验证的恢复密钥。建议用于启用交叉签名的 E2EE 设置。 |
...
| `API_SERVER_KEY` | 用于 API 服务器身份验证的 Bearer 令牌。对非环回绑定强制执行。 |
| `API_SERVER_CORS_ORIGINS` | 允许直接调用 API 服务器的逗号分隔浏览器源（例如 `http://localhost:3000,http://127.0.0.1:3000`）。默认：禁用。 |
...
| `API_SERVER_MODEL_NAME` | 在 `/v1/models` 上通告的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`）。对于多用户设置很有用，其中像 Open WebUI 这样的前端每个连接需要不同的模型名称。 |
| `GATEWAY_PROXY_URL` | 要转发消息的远程 Hermes API 服务器的 URL（[代理模式](/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)）。设置后，网关仅处理平台 I/O — 所有代理工作都委托给远程服务器。也可以通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
| `GATEWAY_PROXY_KEY` | 在代理模式下用于与远程 API 服务器身份验证的 Bearer 令牌。必须与远程主机上的 `API_SERVER_KEY` 匹配。 |
...
| `HERMES_TOOL_PROGRESS` | 已弃用的工具进度显示兼容性变量。首选 `config.yaml` 中的 `display.tool_progress`。 |
| `HERMES_TOOL_PROGRESS_MODE` | 已弃用的工具进度模式兼容性变量。首选 `config.yaml` 中的 `display.tool_progress`。 |
...
| `HERMES_TUI_DIR` | 预构建 `ui-tui/` 目录的路径（必须包含 `dist/entry.js` 和填充的 `node_modules`）。被发行版和 Nix 用来跳过首次启动的 `npm install`。 |

## Cron 调度器

| 变量 | 描述 |
|----------|-------------|
| `HERMES_CRON_TIMEOUT` | cron 作业代理运行的不活动超时（秒，默认：600）。代理可以在主动调用工具或接收流令牌时无限运行 — 这仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | cron 作业附加的预运行脚本的超时（秒，默认：120）。对于需要更长执行时间的脚本（例如反机器人计时的随机延迟）进行覆盖。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `SESSION_RESET_HOUR` | 24 小时格式的每日重置小时（默认：4 = 凌晨 4 点） |

## 上下文压缩（仅 config.yaml）

上下文压缩完全通过 `config.yaml` 配置 — 没有用于它的环境变量。阈值设置位于 `compression:` 块中，而摘要模型/提供者位于 `auxiliary.compression:` 下。

```yaml
compression:
  enabled: true
  threshold: 0.50
  target_ratio: 0.20         # 要保留为最近尾部的阈值的分数
...
对于任务特定的直接端点，Hermes 使用任务配置的 API 密钥或 `OPENAI_API_KEY`。它不会为那些自定义端点重用 `OPENROUTER_API_KEY`。

## 回退模型（仅 config.yaml）

主要模型回退完全通过 `config.yaml` 配置 — 没有用于它的环境变量。添加带有 `provider` 和 `model` 键的 `fallback_model` 部分，以在您的主模型遇到错误时启用自动故障转移。
...
这些放在 `~/.hermes/config.yaml` 中的 `provider_routing` 部分下：

| 键 | 描述 |
|-----|-------------|
| `sort` | 排序提供者：`"price"`（默认）、`"throughput"` 或 `"latency"` |
| `only` | 要允许的提供者 slug 列表（例如 `["anthropic", "google"]`） |
| `ignore` | 要跳过的提供者 slug 列表 |
| `order` | 要按顺序尝试的提供者 slug 列表 |
| `require_parameters` | 仅使用支持所有请求参数的提供者（`true`/`false`） |
| `data_collection` | `"allow"`（默认）或 `"deny"` 以排除数据存储提供者 |

:::tip
使用 `hermes config set` 设置环境变量 — 它会自动将它们保存到正确的文件（秘密保存到 `.env`，其他所有内容保存到 `config.yaml`）。
:::
