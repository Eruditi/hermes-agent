...
Hermes Agent 与任何兼容 OpenAI 的 API 一起工作。支持的提供商包括：
...
使用 `hermes model` 或编辑 `~/.hermes/.env` 设置您的提供商。有关所有提供商密钥，请参阅 [环境变量](./environment-variables.md) 参考。

### 它在 Windows 上工作吗？

**不是原生的**。Hermes Agent 需要类 Unix 环境。在 Windows 上，安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运行 Hermes。标准安装命令在 WSL2 中完美工作：
...
重要说明：完整的 `.[all]` 附加功能目前在 Android 上不可用，因为 `voice` 附加功能依赖于 `faster-whisper` → `ctranslate2`，而 `ctranslate2` 不发布 Android 轮。请改用经过测试的 `.[termux]` 附加功能。

### 我的数据会发送到哪里？

API 调用仅发送到**您配置的 LLM 提供商**（例如，OpenRouter、您的本地 Ollama 实例）。Hermes Agent 不收集遥测、使用数据或分析。您的对话、记忆和技能存储在本地的 `~/.hermes/` 中。

### 我可以离线使用它 / 使用本地模型吗？

是的。运行 `hermes model`，选择 **Custom endpoint**，并输入服务器的 URL：
...
# Context length: 32768   ← 将此设置为匹配您服务器的实际上下文窗口
```
...
Hermes 将端点、提供商和基础 URL 持久化到 `config.yaml` 中，因此它在重启后仍然存在。如果您的本地服务器恰好加载了一个模型，`/model custom` 会自动检测它。您也可以在 config.yaml 中设置 `provider: custom` — 它是一流的提供商，不是其他任何东西的别名。
...
:::tip 本地模型的超时
Hermes 自动检测本地端点并放宽流式传输超时（读取超时从 120s 提高到 1800s，禁用陈旧流检测）。如果在非常大的上下文中仍然遇到超时，请在 `.env` 中设置 `HERMES_STREAM_READ_TIMEOUT=1800`。有关详细信息，请参阅 [本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::

### 它需要多少钱？

Hermes Agent 本身是**免费和开源**的（MIT 许可证）。您只需为所选提供商的 LLM API 使用付费。本地模型完全免费运行。

### 多人可以使用一个实例吗？

是的。[消息网关](../user-guide/messaging/index.md) 允许多个用户通过 Telegram、Discord、Slack、WhatsApp 或 Home Assistant 与同一个 Hermes Agent 实例交互。访问通过允许列表（特定用户 ID）和 DM 配对（第一个发消息的用户获得访问权限）控制。

### 记忆和技能有什么区别？

- **记忆** 存储**事实** — 代理知道的关于您、您的项目和偏好的事情。记忆根据相关性自动检索。
- **技能** 存储**程序** — 如何做事的逐步说明。当代理遇到类似任务时，技能会被召回。
...
安装程序会自动处理这一点 — 如果您在手动安装过程中看到此错误，请先升级 Python。
...
**解决方案：**
```bash
# 不要使用 sudo 与安装程序 — 它安装到 ~/.local/bin
# 如果您之前使用 sudo 安装，请清理：
...
**原因：** `/model`（在聊天会话中）只能在您**已经配置**的提供商之间切换。如果您只设置了 OpenRouter，`/model` 只会显示这些。

**解决方案：** 退出会话并使用终端中的 `hermes model` 添加新提供商：
...
通过 `hermes model` 添加新提供商后，开始新的聊天会话 — `/model` 现在将显示所有已配置的提供商。
...
:::warning
确保密钥与提供商匹配。OpenAI 密钥不适用于 OpenRouter，反之亦然。检查 `~/.hermes/.env` 是否有冲突条目。
:::
...
**原因：** 对话对模型的上下文窗口来说太长，或者 Hermes 为您的模型检测到了错误的上下文长度。
...
如果这发生在第一次长对话中，Hermes 可能对您的模型有错误的上下文长度。检查它检测到了什么：
...
**解决方案：** 当提示时，审查命令并输入 `y` 批准它。您也可以：
- 要求代理使用更安全的替代方案
- 在 [安全文档](../user-guide/security.md) 中查看危险模式的完整列表

:::tip
这是按预期工作的 — Hermes 从不静默运行破坏性命令。批准提示向您显示将执行的内容。
:::
...
:::tip Windows 启动时自动启动
要可靠地自动启动，请使用 Windows 任务计划程序在登录时启动 WSL + 网关：
...
**解决方案：** 当您运行 `hermes gateway install` 时，网关捕获您的 shell PATH。如果您在设置网关后安装了工具，请重新运行安装以捕获更新的 PATH：
...
**原因：** 服务器已启动但工具发现失败，工具被配置过滤，或服务器不支持您预期的 MCP 功能。
...
### 配置文件与仅设置 HERMES_HOME 有什么不同？

配置文件是 `HERMES_HOME` 之上的管理层。您*可以*在每个命令前手动设置 `HERMES_HOME=/some/path`，但配置文件为您处理所有管道：创建目录结构、生成 shell 别名（`hermes-work`）、在 `~/.hermes/active_profile` 中跟踪活动配置文件，以及自动跨所有配置文件同步技能更新。它们还与标签完成集成，因此您不必记住路径。

### 两个配置文件可以共享同一个机器人令牌吗？

不。每个消息平台（Telegram、Discord 等）需要对机器人令牌的独占访问权。如果两个配置文件尝试同时使用同一个令牌，第二个网关将无法连接。为每个配置文件创建一个单独的机器人 — 对于 Telegram，与 [@BotFather](https://t.me/BotFather) 交谈以创建其他机器人。

### 配置文件共享内存或会话吗？

不。每个配置文件都有自己的内存存储、会话数据库和技能目录。它们完全隔离。如果您想使用现有记忆和会话开始新配置文件，请使用 `hermes profile create newname --clone-all` 从当前配置文件复制所有内容。

### 当我运行 `hermes update` 时会发生什么？

`hermes update` 拉取最新代码并**一次**重新安装依赖项（不是每个配置文件）。然后它自动将更新的技能同步到所有配置文件。您只需运行 `hermes update` 一次 — 它覆盖机器上的每个配置文件。
...
导入的配置文件将包含导出的所有配置、记忆、会话和技能。如果新机器有不同的设置，您可能需要更新路径或重新向提供商认证。

### 我可以运行多少个配置文件？

没有硬限制。每个配置文件只是 `~/.hermes/profiles/` 下的一个目录。实际限制取决于您的磁盘空间和系统可以处理的并发网关数量（每个网关是轻量级 Python 进程）。运行几十个配置文件是可以的；每个空闲配置文件不使用资源。
...
现在，当您告诉 Hermes "为 X 写一个 Twitter 线程" 并生成 `delegate_task` 子代理时，该子代理在 Gemini 上运行，而不是您的主模型。您的主要对话保持在 GPT-5.4 上。

您也可以在提示中明确：*"委托一项任务来写关于我们产品发布的社交媒体帖子。使用您的子代理进行实际写作。"* 代理将使用 `delegate_task`，它会自动获取委托配置。

对于没有委托的一次性模型切换，在 CLI 中使用 `/model`：
...
**当前限制：** Hermes 配置文件每个都需要自己的 WhatsApp 号码/会话。您不能将多个配置文件绑定到同一个 WhatsApp 号码上的不同聊天 — WhatsApp 桥接（Baileys）每个号码使用一个认证会话。

**解决方法：**

1. **使用带有个性切换的单个配置文件。** 创建不同的 `AGENTS.md` 上下文文件或使用 `/personality` 命令来更改每个聊天的行为。代理看到它在哪个聊天中，可以适应。

2. **使用 cron 作业执行专门任务。** 对于购物清单跟踪器，设置一个 cron 作业，监控特定聊天并管理清单 — 不需要单独的代理。

3. **使用单独的号码。** 如果您需要真正独立的代理，将每个配置文件与自己的 WhatsApp 号码配对。来自 Google Voice 等服务的虚拟号码适用于此。

4. **改用 Telegram 或 Discord。** 这些平台更自然地支持每聊天绑定 — 每个 Telegram 组或 Discord 频道获得自己的会话，您可以在同一个账户上运行多个机器人令牌（每个配置文件一个）。
...
对于消息平台，`off` 或 `new` 通常是您想要的。编辑 `config.yaml` 后，重新启动网关以使更改生效。

您也可以使用 `/verbose` 命令按会话切换（如果启用）：
...
**场景：** Telegram 有 100 个斜杠命令限制，您的技能正在突破它。您想在 Telegram 上禁用不需要的技能，但 `hermes skills config` 设置似乎不起作用。
...
:::tip
描述很长的技能在 Telegram 菜单中会被截断为 40 个字符，以保持在有效负载大小限制内。如果技能未出现，可能是总有效负载大小问题，而不是 100 个命令计数限制 — 禁用未使用的技能对两者都有帮助。
:::

### 共享线程会话（多个用户，一个对话）

**场景：** 您有一个 Telegram 或 Discord 线程，多人提及机器人。您希望该线程中的所有提及都成为一个共享对话的一部分，而不是每个用户的单独会话。

**当前行为：** Hermes 在大多数平台上按用户 ID 创建会话，因此每个人都获得自己的对话上下文。这是为了隐私和上下文隔离而设计的。

**解决方法：**

1. **使用 Slack。** Slack 会话按线程键控，而不是按用户。同一线程中的多个用户共享一个对话 — 正是您描述的行为。这是最自然的 fit。

2. **使用一个用户的群组聊天。** 如果一个人是指定的"操作员"，负责传递问题，会话保持统一。其他人可以一起阅读。

3. **使用 Discord 频道。** Discord 会话按频道键控，因此同一频道中的所有用户共享上下文。为共享对话使用专用频道。

### 将 Hermes 导出到另一台机器

**场景：** 您在一台机器上构建了技能、cron 作业和记忆，想要将所有内容移动到新的专用 Linux 机器上。
...
3. 在新机器上，运行 `hermes setup` 以验证 API 密钥和提供商配置是否正常工作。重新向任何消息平台认证（特别是 WhatsApp，它使用 QR 配对）。
...
**原因：** 这通常发生在 `~/.zshrc`（或 `~/.bashrc`）具有不正确的文件权限，或安装程序无法干净地写入它时。这不是 Hermes 特定的问题 — 这是 shell 配置权限问题。
